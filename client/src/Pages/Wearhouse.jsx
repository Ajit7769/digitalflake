...
const Wearhouse = () => {
  ...
  const [wearhouseData , setWearhouseData] = useState([]);

  useEffect(()=>{
      const fetchCity = async () => {
          const response = await axios.get("http://localhost:8080/api/allwearhouse");
          console.log(response.data);
          setWearhouseData(response.data.wearhouseData);
        };
        fetchCity();
  },[])
      ...
      <nav className="bg-white border-gray-200 dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <p className="flex items-center space-x-3 rtl:space-x-reverse">
            <span><MdOutlineAddHomeWork className="text-3xl"/></span>
